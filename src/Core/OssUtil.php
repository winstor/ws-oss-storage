...
class OssUtil
{
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && !is_array($options)) {
            throw new WsException ($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            throw new WsException("http body content is invalid");
        }
    }
...
BBB;
    ...
    public static function getHostPortFromEndpoint($endpoint)
    {
        ...
        if (!preg_match('/^[\w.-]+(:[0-9]+)?$/', $str)) {
            throw new WsException("endpoint is invalid:" . $endpoint);
        }
    ...
    public static function decodeKey($key, $encoding)
    {
        ...
        if ($encoding == "url") {
            ...
            throw new WsException("Unrecognized encoding type: " . $encoding);
        }
    }
